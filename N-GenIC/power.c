...
{
  /*  Note that the linear growth factor can be computed from a
      1-D integral for flat LCDM models or for models with curvature
      and no cosmological constant (see Peebles "Physical Cosmology" book,
      and also Linder & Jenkins 2003)
  */
  ...
  if(OmegaLambda != 0)
    {
      if(fabs(1 - Omega - OmegaLambda) > 1.0e-6)
        {
          printf("for OmegaLambda != 0 only flat cosmologies are allowed\n");
          exit(1);
        }
    }
